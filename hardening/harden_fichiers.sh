#!/bin/bash
# MP2 Hardening - Fichiers server (192.168.10.20, VLAN10)
# Covers: SSH, UFW, Apache, MariaDB, PAM/MFA, sudo, rsyslog, fail2ban, rkhunter, aide, backup
# NO OpenLDAP (only on AD-LDAP server)
...
echo "=========================================="
echo " MOBITECH MP2 - Hardening: Fichiers"
echo " $(date)"
echo "=========================================="
...
ufw allow from 192.168.10.0/24 to any port 445 proto tcp comment "Samba VLAN10"
ufw allow from 192.168.20.0/24 to any port 445 proto tcp comment "Samba VLAN20"
ufw allow from 192.168.10.0/24 to any port 139 proto tcp comment "NetBIOS VLAN10"
ufw allow from 192.168.20.0/24 to any port 139 proto tcp comment "NetBIOS VLAN20"
ufw allow from 192.168.10.0/24 to any port 2049 proto tcp comment "NFS VLAN10"
ufw allow from 192.168.20.0/24 to any port 2049 proto tcp comment "NFS VLAN20"
ufw --force enable
ufw status verbose
...
sed -i "s/^bind-address.*/bind-address = $SERVER_IP/" /etc/mysql/mariadb.conf.d/50-server.cnf
...
echo ""
echo "=========================================="
echo " Hardening COMPLETE: Fichiers"
echo " $(date)"
echo "=========================================="
echo "  SSH:2222  UFW:active  MariaDB:$SERVER_IP  fail2ban:active"
